...
let combinerCalls = 0;
const origCmb = rcp.combineColor.bind(rcp);
rcp.combineColor = function(shade, tex) { combinerCalls++; return { r: 200, g: 100, b: 50, a: 255 }; };

// Also count actual setUint16 writes to FB area
let fbWrites = 0;
const origSetU16 = DataView.prototype.setUint16;
DataView.prototype.setUint16 = function(off, v, le) {
    if (off >= 0x38f800 && off < 0x3da800 && v !== 0x0001) {
        fbWrites++;
        if (fbWrites <= 5) console.log(`FB write @ 0x${off.toString(16)} val=0x${v.toString(16)}`);
    }
...
console.log('combinerCalls:', combinerCalls, 'tri:', rcp.drawStats.triangles, 'f3d:', rcp.f3dTaskCount);
console.log('FB writes (non-0x0001):', fbWrites);
