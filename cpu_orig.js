class CPU {
    ...
    performHleBoot() {
        ...
        this.gpr[1]  = BigInt.asIntN(64, 0x0000000000000001n); // at
        this.gpr[2]  = BigInt.asIntN(64, 0x000000000EBDA536n); // v0
        this.gpr[3]  = BigInt.asIntN(64, 0x000000000EBDA536n); // v1
        this.gpr[4]  = BigInt.asIntN(64, 0x000000000000A536n); // a0
        this.gpr[5]  = BigInt.asIntN(64, 0xFFFFFFFFC0F1D859n); // a1
        this.gpr[6]  = BigInt.asIntN(64, 0xFFFFFFFFA4001F0Cn); // a2
        this.gpr[7]  = BigInt.asIntN(64, 0xFFFFFFFFA4001F08n); // a3
        this.gpr[8]  = BigInt.asIntN(64, 0x00000000000000C0n); // t0
        this.gpr[10] = BigInt.asIntN(64, 0x0000000000000040n); // t2
        this.gpr[11] = BigInt.asIntN(64, 0xFFFFFFFFA4000040n); // t3
        this.gpr[12] = BigInt.asIntN(64, 0xFFFFFFFFED10D0B3n); // t4
        this.gpr[13] = BigInt.asIntN(64, 0x000000001402A4CCn); // t5
        this.gpr[14] = BigInt.asIntN(64, 0x000000002DE108EAn); // t6
        this.gpr[15] = BigInt.asIntN(64, 0x000000003103E121n); // t7
        this.gpr[20] = 1n; // s4 - TV type (1 = PAL on Euro carts; 0 = NTSC)
        this.gpr[22] = BigInt(cic.seed & 0xFF); // s6 (CIC seed)
        this.gpr[23] = 0n; // s7
        this.gpr[25] = BigInt.asIntN(64, 0xFFFFFFFF9DEBB54Fn); // t9
        this.gpr[29] = BigInt.asIntN(64, 0xFFFFFFFFA4001FF0n); // sp
        this.gpr[31] = BigInt.asIntN(64, 0xFFFFFFFFA4001550n); // ra

        // CIC-specific tweaks per real-hardware traces.
        if (cic.name.indexOf('6101') >= 0) {
            this.gpr[20] = 1n;
            this.gpr[27] = BigInt.asIntN(64, 0xFFFFFFFF8DA807FCn); // k1
        } else if (cic.name.indexOf('6103') >= 0 || cic.name.indexOf('7103') >= 0) {
            this.gpr[20] = 0n;
            this.gpr[17] = BigInt.asIntN(64, 0xFFFFFFFFD4646273n); // s1 (CIC-6103 marker)
            this.gpr[27] = BigInt.asIntN(64, 0xFFFFFFFFD4646273n); // k1
        } else if (cic.name.indexOf('6105') >= 0) {
            this.gpr[20] = 0n;
            this.gpr[31] = BigInt.asIntN(64, 0xFFFFFFFFA4001550n); // ra
        } else if (cic.name.indexOf('6106') >= 0) {
            this.gpr[20] = 0n;
            this.gpr[31] = BigInt.asIntN(64, 0xFFFFFFFFA4001554n); // ra
        }
        ...
        this.pc = BigInt.asIntN(32, BigInt(entryPoint));
    ...
    readInstructionWord(pc) {
        const pcNum = Number(pc & 0xFFFFFFFFn);
    ...
    step() {
        ...
        if ((this.pc & 0xFFFFFFFFn) === 0x802ef780n) {
            ...
            if (this._spinCallCount <= 3 || f3d >= 96) {
                ...
                const ra = Number(this.gpr[31] & 0xFFFFFFFFn);
                /* [spinwatch] silenced */
            } }
        }
        // OVERLAY ENTRY TRAP: log first 5 calls to 0x8016f000 (overlay init)
        { const _opc = this.pc & 0xFFFFFFFFn;
          if (_opc === 0x8016f000n) {
              ...
              if (this._overlayCallCount <= 5) {
                  const f3d = this.mmu && this.mmu.rcp ? (this.mmu.rcp.f3dTaskCount|0) : -1;
                  const ra = Number(this.gpr[31] & 0xFFFFFFFFn);
                  const a0 = Number(this.gpr[4] & 0xFFFFFFFFn);
                  const totalDma = this.mmu ? (this.mmu._totalPiDmaCount|0) : -1;
                  /* silenced */
              }
          }
        ...
        if (this.mmu && this.mmu.rcp && (this.mmu.rcp.f3dTaskCount|0) >= 96) {
            const _pc96 = this.pc & 0xFFFFFFFFn;
            if (_pc96 === 0x80242bb0n) {
                ...
                if (this._stateDumpCount <= 6) {
                    ...
                    const sp = Number(this.gpr[29] & 0xFFFFFFFFn);
        ...
        if (this.branchTaken) {
            const ds = this.readInstructionWord(currentPc + 4n);
