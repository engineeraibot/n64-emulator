...
// Force shade-only by neutralizing combiner and tex
rcp.sampleTexture = function() { return { r: 255, g: 255, b: 255, a: 255 }; };
rcp.combineColor = function(shade, tex) {
    return { r: Math.min(255, shade.r), g: Math.min(255, shade.g), b: Math.min(255, shade.b), a: 255 };
};
...
console.log('[shadeonly] f3d:', rcp.f3dTaskCount, 'tri:', rcp.drawStats.triangles, 'offscreen:', rcp.drawStats.offscreenTriangles||0);

function decode5551(v){return{r:((v>>11)&0x1F)<<3,g:((v>>6)&0x1F)<<3,b:((v>>1)&0x1F)<<3,a:(v&1)?255:0};}
function countNB(o){const rd=new DataView(ram.rdram);let nb=0;for(let y=0;y<240;y++)for(let x=0;x<320;x++){const a=(o+(y*320+x)*2)&0x7FFFFF;const v=(rd.getUint8(a)<<8)|rd.getUint8((a+1)&0x7FFFFF);const c=decode5551(v);if(c.r||c.g||c.b)nb++;}return nb;}
function dumpFB(o,p){const rd=new DataView(ram.rdram);const out=Buffer.alloc(FB_W*FB_H*4);let q=0;
  for(let y=0;y<240;y++)for(let x=0;x<320;x++){const a=(o+(y*320+x)*2)&0x7FFFFF;const v=(rd.getUint8(a)<<8)|rd.getUint8((a+1)&0x7FFFFF);const c=decode5551(v);out[q++]=c.r;out[q++]=c.g;out[q++]=c.b;out[q++]=255;}
...
for(const fb of [0x38f800, 0x3b5000, 0x3da800]) {
    console.log(`[shadeonly] FB 0x${fb.toString(16)}: ${countNB(fb)} non-black`);
    dumpFB(fb, path.join(ROOT, 'test-results', `sm64-shadeonly-${fb.toString(16)}.png`));
}
