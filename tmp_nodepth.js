...
// Report depthImage values seen, and optionally force-disable depth.
const DISABLE=process.env.NODEPTH==='1';
const diSeen={};
const origRast=rcp.rasterizeTriangle.bind(rcp);
rcp.rasterizeTriangle=function(v1,v2,v3,addr){
  const di=(rcp.rspState.depthImage>>>0);diSeen[di.toString(16)]=(diSeen[di.toString(16)]||0)+1;
  if(DISABLE)rcp.rspState.depthImage=0;
...
realLog('depthImage histogram:',JSON.stringify(diSeen));
const snap=rcp.bestRichVideoSnapshot;
realLog('snap origin=0x'+(snap.origin>>>0).toString(16),'nonBlack='+snap.nonBlack);
...
writePng(rgba,W,H,process.env.OUT||'test-results/nodepth.png');
realLog('wrote',process.env.OUT||'test-results/nodepth.png');
