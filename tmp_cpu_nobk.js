class CPU {
    ...
    step() {
        ...
        if ((this.instructionCount & 0x7F) === 0) {
            this.cp0Registers[9] = (this.cp0Registers[9] + 64) | 0;
            this.serviceCompareTimer();
            this.mmu.checkInternalEvents();
            const miIntr = this.mmu.miRegisters[2];
            const miMask = this.mmu.miRegisters[3];
            if (miIntr & miMask) this.cp0Registers[13] |= 0x00000400; else this.cp0Registers[13] &= ~0x00000400;
            const st2 = this.cp0Registers[12];
            const ca2 = this.cp0Registers[13];
            if ((st2 & 1) && !(st2 & 2) && !(st2 & 4) && ((st2 >> 8) & (ca2 >> 8) & 0xFF)) this.raiseException(0, this.pc, false);
        }
