...
// EXPERIMENT: only flat-fill in FILL(3)/COPY(2) cycle modes
const oF=rcp.handleG_FILLRECT.bind(rcp);
rcp.handleG_FILLRECT=function(hi,lo){const cyc=(this.rspState.otherModeHi>>>20)&3;if(cyc===0||cyc===1){return;}return oF(hi,lo);};
...
rcp.processDisplayList=function(addr,ds){const r=oP(addr,ds);const ci=this.rspState.colorImage>>>0;const nb=scan(ci);if(nb>best){best=nb;bestA=ci&0x7FFFFF;}return r;};
const startF=rcp.f3dex2TaskCount|0;const t0=Date.now();
for(let s=0;s<300000000;s++){try{cpu.step();}catch(e){log('THREW',e.message);break;}
  if((s&0x3FFF)===0){if((rcp.f3dex2TaskCount|0)-startF>=120)break;if(Date.now()-t0>32000)break;}}
log('best rendered nb='+best+' at 0x'+bestA.toString(16));
...
const W=320,H=240,o2=Buffer.alloc(W*H*4);let p=0;for(let i=0;i<W*H;i++){const q=(bestA+i*2)&0x7FFFFF;const v=(rd[q]<<8)|rd[q+1];o2[p++]=((v>>11)&31)<<3;o2[p++]=((v>>6)&31)<<3;o2[p++]=((v>>1)&31)<<3;o2[p++]=255;}
fs.mkdirSync('test-results',{recursive:true});wpng(o2,W,H,process.env.OUT_PNG||'test-results/oottest.png');log('wrote',process.env.OUT_PNG);
