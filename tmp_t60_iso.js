...
const N=parseInt(process.env.N||'40000000',10);
const SKIPVAL=process.env.SKIPVAL==='1';
const m=build(ROM);loadState(STATE,m.ram,m.mmu,m.cpu,m.rcp);m.cpu.useJit=true;
const cpu=m.cpu;
if(SKIPVAL) cpu.validateBlock=function(){return true;};
...
function drive(t){ while(cpu.instructionCount<t) cpu.stepJit(); }
...
console.log('JIT'+(SKIPVAL?'(no-validate)':'(validate)'),STATE,'real-rate',(real/dt/1e6).toFixed(3)+'M/s','('+dt.toFixed(2)+'s,',(real/1e6).toFixed(2)+'M real)');
