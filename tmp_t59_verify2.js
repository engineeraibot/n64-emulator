...
const A=build('cpu_pre_t59_backup.js','mmu_pre_t59_backup.js',ROM);loadState(STATE,A.ram,A.mmu,A.cpu,A.rcp);
const B=build('cpu.js','mmu.js',ROM);loadState(STATE,B.ram,B.mmu,B.cpu,B.rcp);
let div=-1;
for(let i=0;i<N;i++){A.cpu.step();B.cpu.step();if((i&0x3FFFF)===0&&A.cpu.pc!==B.cpu.pc){div=i;break;}}
const ca=crc(A.ram.rdram),cb=crc(B.ram.rdram);
console.log(STATE,'N',N,'crc',ca===cb?'IDENTICAL':('DIFFER '+ca.toString(16)+' '+cb.toString(16)),'pc',A.cpu.pc===B.cpu.pc?'same':'DIFF','ic',A.cpu.instructionCount===B.cpu.instructionCount?'same':'DIFF',div>=0?('DIVERGED@'+div):'');
