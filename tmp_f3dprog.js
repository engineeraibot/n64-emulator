...
const MAX=parseInt(process.env.MAX||'28000000');
let last=0;
for(let s=0;s<MAX;s++){
  const f=rcp.f3dTaskCount|0;
  if(f!==last){ if(f<=8||f%16===0) realLog('f3d',f,'@step',s); last=f; }
  cpu.step();
}
realLog('FINAL f3d=',rcp.f3dTaskCount|0,'tri=',rcp.drawStats&&rcp.drawStats.triangles,'@',MAX);
