...
const hist={};
const START=parseInt(process.env.S||'12000000'),N=parseInt(process.env.N||'4000000');
for(let s=0;s<START+N;s++){ if(s>=START){const pc=cpu.pc>>>0;hist[pc]=(hist[pc]||0)+1;} cpu.step(); }
const top=Object.entries(hist).sort((a,b)=>b[1]-a[1]).slice(0,15);
for(const[pc,n]of top)console.log('0x'+(+pc).toString(16),n);
