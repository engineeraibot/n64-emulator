...
function dis(w,va){const op=w>>>26,rs=(w>>21)&31,rt=(w>>16)&31,rd=(w>>11)&31,sa=(w>>6)&31,fn=w&63,imm=(w<<16>>16);
 if(w===0)return 'nop';
 if(op===0){const m={0x08:'jr',0x09:'jalr',0x21:'addu',0x23:'subu',0x25:'or',0x24:'and',0x2a:'slt',0x2b:'sltu',0x10:'mfhi',0x12:'mflo'};
   if(fn===0x08)return 'jr '+R[rs]; if(fn===0x09)return 'jalr '+R[rd]+','+R[rs];
   if(fn===0x00)return 'sll '+R[rd]+','+R[rt]+','+sa;
   return (m[fn]||('spec fn=0x'+fn.toString(16)))+' '+R[rd]+','+R[rs]+','+R[rt];}
 if(op===0x10)return 'cop0';
 if(op===0x11)return 'cop1 rs=0x'+rs.toString(16)+' ft='+rt+' fs='+rd+' fd='+sa+' fn=0x'+fn.toString(16);
 ...
 const m={0x23:'lw',0x24:'lbu',0x20:'lb',0x25:'lhu',0x21:'lh',0x2b:'sw',0x28:'sb',0x29:'sh',0x0f:'lui',0x0d:'ori',0x09:'addiu',0x0c:'andi',0x0a:'slti',0x0b:'sltiu',0x31:'lwc1',0x39:'swc1',0x35:'ldc1',0x3d:'sdc1'};
...
for(let s=0;s<MAX;s++){
  ...
  if(pc===0x8018c2c8 && !done){
    ...
    if(/does not support/.test(fmt)){
      ...
      const objp=cpu.gpr[6]>>>0;
      console.log('PANIC at step',s,'fmt="'+fmt+'" funcName="'+str(cpu.gpr[5])+'"');
      console.log('object ptr=0x'+objp.toString(16));
      console.log(' obj words:');
      for(let o=0;o<0x30;o+=4)console.log('   +0x'+o.toString(16)+' = 0x'+rd(objp+o).toString(16)+'  ("'+str(rd(objp+o))+'")');
      console.log('--- caller disasm around 0x80188204 ---');
      for(let va=0x801881a0;va<0x80188240;va+=4)console.log('  0x'+va.toString(16),dis(rd(va),va));
      break;
    }
  }
  cpu.step();
