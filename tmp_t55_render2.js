...
// capture each colorImage buffer right after the frame that drew the most tris into it
...
rcp.processDisplayList=function(a,d){const before=curTris;const r=oP(a,d);const drew=curTris-before;
  if(drew>0){const ci=curCi;const prev=perCi.get(ci)||{tris:0};if(drew>prev.tris){perCi.set(ci,{tris:drew,buf:Buffer.from(Buffer.from(rd.buffer,ci,320*240*2))});}}
  return r;};
...
for(const [ci,e] of perCi){const nb=nbScan(e.buf);log('colorImage 0x'+ci.toString(16),'maxTris',e.tris,'nb',nb);if(nb>bestNb){bestNb=nb;best=e.buf;}}
...
if(best){const W=320,H=240,o2=Buffer.alloc(W*H*4);let p=0;for(let i=0;i<W*H;i++){const v=(best[i*2]<<8)|best[i*2+1];o2[p++]=((v>>11)&31)<<3;o2[p++]=((v>>6)&31)<<3;o2[p++]=((v>>1)&31)<<3;o2[p++]=255;}fs.mkdirSync('test-results',{recursive:true});wpng(o2,W,H,process.env.OUT_PNG||'test-results/oot_r2.png');log('wrote',process.env.OUT_PNG,'bestNb',bestNb);}
