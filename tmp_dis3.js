...
function dis(w){const op=w>>>26,rs=(w>>21)&31,rt=(w>>16)&31,rd=(w>>11)&31,sa=(w>>6)&31,fn=w&63,imm=(w<<16>>16);
 if(w===0)return 'nop';
 if(op===0){const m={0x08:'jr',0x09:'jalr',0x21:'addu',0x20:'add',0x23:'subu',0x25:'or',0x24:'and',0x2a:'slt',0x2b:'sltu',0x00:'sll',0x02:'srl',0x03:'sra'};if(m[fn])return m[fn]+' '+R[rd]+','+R[rs]+','+R[rt];return 'spec fn=0x'+fn.toString(16);}
 if(op===0x10){if((w&0x3f)===0x18)return 'eret';const sub=(w>>21)&31;return (sub===0?'mfc0 '+R[rt]+',$'+rd:sub===4?'mtc0 '+R[rt]+',$'+rd:'cop0');}
 const m={0x23:'lw',0x20:'lb',0x24:'lbu',0x25:'lhu',0x21:'lh',0x37:'ld',0x2b:'sw',0x28:'sb',0x29:'sh',0x3f:'sd',0x0f:'lui',0x0d:'ori',0x09:'addiu',0x0c:'andi',0x0a:'slti',0x0b:'sltiu',0x0e:'xori'};
 if(op===0x04)return 'beq '+R[rs]+','+R[rt]+',.'+(imm*4);
 if(op===0x05)return 'bne '+R[rs]+','+R[rt]+',.'+(imm*4);
 if(op===0x14)return 'beql '+R[rs]+','+R[rt]+',.'+(imm*4);
 if(op===0x15)return 'bnel '+R[rs]+','+R[rt]+',.'+(imm*4);
 if(op===0x01){const sub=(w>>16)&31;return 'regimm sub=0x'+sub.toString(16)+' '+R[rs]+',.'+(imm*4);}
 ...
 if(m[op]){if(op===0x0f)return 'lui '+R[rt]+',0x'+(w&0xffff).toString(16);if(op>=0x20||op===0x37||op===0x3f)return m[op]+' '+R[rt]+','+imm+'('+R[rs]+')';return m[op]+' '+R[rt]+','+R[rs]+','+imm;}
 return 'op=0x'+op.toString(16);}
for(const base of [0x802ef610,0x802f1180,0x802df700]){
 console.log('=== 0x'+base.toString(16)+' ===');
 for(let va=base;va<base+0x40;va+=4){const w=ram.read32(va&0x7FFFFF)>>>0;console.log('  0x'+va.toString(16),dis(w));}
}
