...
const quiet={log:()=>{}};
const sb={console:quiet,setTimeout:()=>{},clearTimeout:()=>{},performance:{now:()=>Date.now()},Math,Number,BigInt,JSON,DataView,ArrayBuffer,Uint8Array,Uint16Array,Uint32Array,Int8Array,Int16Array,Int32Array,Float32Array,Float64Array,Array};
...
for(let s=0;s<MAX;s++){
  ...
  cpu.step();
  const cur=rd(WATCH);
  if(cur!==last){ realLog('WRITE step',s,'pc=0x'+pc.toString(16),'0x'+last.toString(16),'->0x'+cur.toString(16)); last=cur; writes++; if(writes>30){realLog('...stop');break;} }
}
realLog('done. total writes to 0x9a3f4 =',writes,' final=0x'+rd(WATCH).toString(16));
