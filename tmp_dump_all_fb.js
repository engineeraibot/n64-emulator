...
console.log('[fb] f3d:', rcp.f3dTaskCount, 'tri:', rcp.drawStats.triangles);
function decode5551(v) { return { r: ((v>>11)&0x1F)<<3, g: ((v>>6)&0x1F)<<3, b: ((v>>1)&0x1F)<<3, a: (v&1)?255:0 }; }
function countNB(origin) {
    const rd = new DataView(ram.rdram); let nb = 0;
    for (let y=0;y<FB_H;y++) for (let x=0;x<FB_W;x++) {
        const a = (origin + (y*320+x)*2) & 0x7FFFFF;
...
function dumpFB(origin, p) {
    const rd = new DataView(ram.rdram);
    const out = Buffer.alloc(FB_W*FB_H*4); let q=0;
    for (let y=0;y<FB_H;y++) for (let x=0;x<FB_W;x++) {
        const a = (origin + (y*320+x)*2) & 0x7FFFFF;
        const v = (rd.getUint8(a)<<8) | rd.getUint8((a+1)&0x7FFFFF);
        const c = decode5551(v);
        out[q++]=c.r;out[q++]=c.g;out[q++]=c.b;out[q++]=255;
    }
    function crc32(buf){const T=new Uint32Array(256);for(let n=0;n<256;n++){let c=n;for(let k=0;k<8;k++)c=(c&1)?(0xEDB88320^(c>>>1)):(c>>>1);T[n]=c;}let cr=0xFFFFFFFF;for(let n=0;n<buf.length;n++)cr=T[(cr^buf[n])&0xFF]^(cr>>>8);return (cr^0xFFFFFFFF)>>>0;}
...
const fbs = [0x38f800, 0x3b5000, 0x3da800];
for (const fb of fbs) {
    const nb = countNB(fb);
    console.log(`[fb] FB 0x${fb.toString(16)}: ${nb} non-black`);
    dumpFB(fb, path.join(ROOT, 'test-results', `sm64-fb-${fb.toString(16)}.png`));
}
const viOrigin = mmu.viRegisters[1] & 0x7FFFFF;
console.log(`[fb] VI origin: 0x${viOrigin.toString(16)}`);
console.log('[fb] colorImage at end:', '0x' + (rcp.rspState.colorImage>>>0).toString(16));
