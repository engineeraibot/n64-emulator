...
const CUR=0x801a7784&0x7FFFFF;
const MAX=26200000, WARM=25800000;
for(let s=0;s<WARM;s++)cpu.step();
let last=rd(CUR);
realLog('warm; current sCurrentMoveObj=0x'+last.toString(16));
for(let s=WARM;s<MAX;s++){
  const pc=cpu.pc>>>0;
  cpu.step();
  const cur=rd(CUR);
  if(cur!==last){
    const t=cur?('type=0x'+rd(cur+0xC).toString(16)+' id='+rh(cur+0x10)):'(null)';
    realLog('CUR write step',s,'pc=0x'+pc.toString(16),'0x'+last.toString(16),'->0x'+cur.toString(16),t);
    last=cur;
  }
