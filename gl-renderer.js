...
vec4 fetchTexel(vec2 st) {
  float ts = texAddr(floor(st.x), uAddrS.x, uAddrS.y, uAddrS.z);
  float tt = texAddr(floor(st.y), uAddrT.x, uAddrT.y, uAddrT.z);
...
class N64GLRenderer {
    constructor(canvas) {
        ...
        for (const n of ['uFbSize','uTex','uTexSize','uUseTex','uAddrS','uAddrT','uMode','uC2',
                         ...
                         'uAlphaGate','uCvgSel','uCopyGate','uBl0On','uBl0','uBl1On','uBl1']) {
    ...
    _stateFor(rcp, kind, tileIdx) {
        ...
        const useTex = kind === 1 ? (rs.textureImage !== 0) : rs.useTexture;
        ...
        const s = {
            ...
            alphaGate: copy ? 0 : (kind === 1 ? (blActive ? 0 : 1) : ((rs.otherModeLo & 0x4000) ? 1 : 0)),
