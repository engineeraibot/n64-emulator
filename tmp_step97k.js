...
for(let s=0;s<97020;s++)cpu.step();
for(let s=97020;s<97035;s++){
  const pc=cpu.pc>>>0,phys=pc&0x7FFFFF,instr=ram.read32(phys)>>>0;
  const op=instr>>>26, fn=instr&0x3f;
  console.log('step',s,'PC=0x'+pc.toString(16),'I=0x'+instr.toString(16).padStart(8,'0'),'op=0x'+op.toString(16),'fn=0x'+fn.toString(16),
    'hi=0x'+(cpu.hi>>>0).toString(16),'lo=0x'+(cpu.lo>>>0).toString(16));
  cpu.step();
}
