...
function dis(w){const op=w>>>26,rs=(w>>21)&31,rt=(w>>16)&31,rd2=(w>>11)&31,sa=(w>>6)&31,fn=w&63,imm=(w<<16>>16);
 ...
 if(op===0){
   ...
   const m={0x21:'addu',0x23:'subu',0x25:'or',0x24:'and',0x2a:'slt',0x2b:'sltu',0x10:'mfhi',0x12:'mflo',0x06:'srlv',0x04:'sllv',0x07:'srav',0x2d:'daddu',0x26:'xor',0x27:'nor'};
...
function dump(start,n){ for(let i=0;i<n;i++){const va=start+i*4;realLog('0x'+(va>>>0).toString(16)+': '+dis(rd(va)));} realLog('----'); }
for(let s=0;s<26000000;s++)cpu.step();
const START=parseInt(process.env.A,16), N=parseInt(process.env.N||'48');
realLog('=== 0x'+START.toString(16)+' ===');dump(START,N);
function rstr(a){let s='';a&=0x7FFFFF;for(let i=0;i<80;i++){const ch=b[a+i];if(!ch)break;s+=String.fromCharCode(ch);}return s;}
realLog('str@0x801b55c0:',JSON.stringify(rstr(0x801b55c0)));
realLog('str@0x801b4ecc:',JSON.stringify(rstr(0x801b4ecc)));
realLog('str@0x801b4dc5:',JSON.stringify(rstr(0x801b4dc5)));
