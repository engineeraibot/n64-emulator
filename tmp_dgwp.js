...
function dis(w,va){const op=w>>>26,rs=(w>>21)&31,rt=(w>>16)&31,rd2=(w>>11)&31,sa=(w>>6)&31,fn=w&63,imm=(w<<16>>16);
 if(w===0)return 'nop';
 if(op===0){const m={0x08:'jr',0x09:'jalr',0x21:'addu',0x23:'subu',0x25:'or',0x24:'and',0x2a:'slt',0x2b:'sltu',0x10:'mfhi',0x12:'mflo',0x06:'srlv',0x04:'sllv',0x07:'srav'};
...
const MAX=27000000;
const START=parseInt(process.env.START||'0x80187d00',16);
const END=parseInt(process.env.END||'0x80187f80',16);
let done=false;
for(let s=0;s<MAX;s++){
  const pc=cpu.pc>>>0;
  if(pc===0x8018c2c8 && /does not support/.test(str(cpu.gpr[4])) && !done){
    done=true;
    console.log('PANIC step',s);
    for(let va=START;va<END;va+=4)console.log('  0x'+va.toString(16),dis(rd(va),va));
    break;
  }
  cpu.step();
}
