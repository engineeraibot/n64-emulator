document.addEventListener('DOMContentLoaded', () => {
    ...
    // main thread). A slow PI controller on the buffer fill adapts playback speed to
    // the emulator's actual production rate (audio slows in pitch with the emu rather
    // than crackling, and converges to 1.0x at full speed). Underruns fade out and
    ...
    function N64AudioPullEngine(outRate) {
        ...
        let inRate = 32000, speed = 1.0, started = false, ramp = 0;
        let lastL = 0, lastR = 0;
        const TARGET = 4096;                    // input frames (~128ms @32kHz)
        const START = 2048;                     // refill level before (re)starting
        const KP = 2e-5, KI = 2e-6;             // fill-error -> speed (overdamped, slow)
        ...
        function pull(L, R, n) {
            if (!started) {
                if (fill < START) { L.fill(0, 0, n); R.fill(0, 0, n); return; }
                started = true; ramp = 0;
            }
            const err = fill - TARGET;
            speed = Math.min(4, Math.max(0.05, speed + err * KI * (n / 128)));
            const ratio = (inRate / outRate) * Math.min(4, Math.max(0.05, speed + err * KP));
            for (let i = 0; i < n; i++) {
                if (fill < 2) {
    ...
    const workletSrc = 'const N64AudioPullEngine = ' + N64AudioPullEngine.toString() + ';\n' +
        ...
        '    this.engine.pull(o[0], o[1] || o[0], o[0].length); return true; }\n' +
        '}\n' +
