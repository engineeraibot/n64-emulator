...
const EV=['SW1','SW2','CART','COUNTER','SP','SI','AI','VI','PI','DP','CPU_BREAK','SP_BREAK','FAULT','THREADSTATUS','PRENMI','RDB_RD','RDB_WR','RDB_BRK',];
const regs=[];
for(let s=0;s<12000000;s++){
  if((cpu.pc>>>0)===0x802ef710){
    const ev=cpu.gpr[4]|0, mq=cpu.gpr[5]>>>0, msg=cpu.gpr[6]>>>0;
    regs.push({ev,mq,msg});
  }
  cpu.step();
}
console.log('osSetEventMesg registrations:');
for(const r of regs)console.log('  event',r.ev,'('+(EV[r.ev]||'?')+')  mq=0x'+r.mq.toString(16)+'  msg=0x'+r.msg.toString(16));
