...
const origin=(mmu.viRegisters[1]>>>0)&0x7FFFFF;
realLog('VI origin=0x'+origin.toString(16),'VI w=',mmu.viRegisters[2]>>>0);
...
const rd=new Uint8Array(ram.rdram),W=320,H=240,rgba=Buffer.alloc(W*H*4);let p=0;
for(let y=0;y<H;y++)for(let x=0;x<W;x++){const i=(origin+(y*W+x)*2)&0x7FFFFF;const v=(rd[i]<<8)|rd[i+1];rgba[p++]=((v>>11)&31)<<3;rgba[p++]=((v>>6)&31)<<3;rgba[p++]=((v>>1)&31)<<3;rgba[p++]=255;}
writePng(rgba,W,H,process.env.OUT);realLog('wrote',process.env.OUT);
