...
function dis(w,va){const op=w>>>26,rs=(w>>21)&31,rt=(w>>16)&31,rd2=(w>>11)&31,sa=(w>>6)&31,fn=w&63,imm=(w<<16>>16);
 if(w===0)return 'nop';
 if(op===0){const m={0x08:'jr',0x09:'jalr',0x21:'addu',0x23:'subu',0x25:'or',0x24:'and',0x2a:'slt',0x2b:'sltu',0x10:'mfhi',0x12:'mflo',0x06:'srlv',0x04:'sllv',0x07:'srav',0x2d:'daddu'};
...
// disasm 0x8017cc44 once we're past overlay load
const MAX=27000000;
let dumped=false;
for(let s=0;s<MAX;s++){
  const pc=cpu.pc>>>0;
  if(pc===0x8017cc44 && !dumped){
    const obj=cpu.gpr[4]>>>0;
    console.log('0x8017cc44 entry: a0=0x'+obj.toString(16),'type@+0xC=0x'+rd(obj+0xC).toString(16),'a1=0x'+(cpu.gpr[5]>>>0).toString(16));
    console.log('--- disasm 0x8017cc44 ---');
    for(let va=0x8017cc44;va<0x8017cd00;va+=4)console.log('  0x'+va.toString(16),dis(rd(va),va));
    dumped=true;
    // continue to find sub-obj type
    console.log('sub-obj 0x8009a3d8 type=0x'+rd(0x8009a3d8+0xC).toString(16));
    break;
  }
  cpu.step();
}
